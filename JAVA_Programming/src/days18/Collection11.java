...
class Rect{
	...
	//Hash 기반의 클래스에 의해 호출되는 메소드를 오버라이딩하여 동일한 형태의 객체를
	// 검색하기 위해 사용되게 함. 동일한 hashCode를 반환하는 경우에만 비교를 수행
	public int hashCode() {
...
public class Collection11 {
	...
	public static void main(String[] args) {
		...
		//Hash 알고리즘을 기반으로 구현된 클래스는 equals 메소드가 호출되지 않는 문제점이 있습니다
		
		// Hash 기반의 클래스에서 키값의 중복을 제거하기 위해 실행되는 과정
		//(키값이 사용자정의 클래스 객체일때)
		// 1. 입력된(검색할) 객체의 hashCode 메소드 실행
		// 2. 반환받은 hashCode 메소드의 결과를 현재 저장되어있는 
		//    모든 객체의 hashCode 메소드의 반환값과 비교
